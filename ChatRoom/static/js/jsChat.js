...
// Вывод комнат в браузер   document.querySelector('.rightdiv').removeChild(document.querySelector('.div_chat'));document.getElementById('.div_chat').innerHTML = ""
...
function jampInRoom(item) {
    ...
    if (socket.readyState === WebSocket.OPEN) {
        ...
        console.error("WebSocket не открыт. readyState: " + socket.readyState);
    }
    currentRoomId = item;
    console.log({'load': 'messageList', 'room_id': item});
    console.log(currentRoomId);

    if (socketForChat.readyState === WebSocket.OPEN) {
        socketForChat.send(JSON.stringify({'usersendcommandroom': 'roomselect', 'newroom_id': item, 'oldroom_id': currentRoomId}));
    } else {
        console.error("WebSocket для чата не открыт. readyState: " + socketForChat.readyState);
    }
...
function jampInUser(item) {
    ...
    if (inputMessage) {
        ...
        console.error("Элемент с классом 'message_box' не найден");
    }
}
